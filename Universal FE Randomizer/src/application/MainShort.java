...
public class MainShort {
	public static void main(String[] args) throws IOException {
		...
		PortraitDataLoader pd = new PortraitDataLoader(FE8Data.portraitProvider, sourceFile);
		...
		CharacterShufflingOptions shufflingOptions = new CharacterShufflingOptions(CharacterShufflingOptions.ShuffleLevelingMode.AUTOLEVEL, true, 100, Arrays.asList("fe6chars.json", "fe7chars.json"));
		...
		ItemAssignmentOptions inventoryOptions = new ItemAssignmentOptions(WeaponReplacementPolicy.ANY_USABLE, ShopAdjustment.ADJUST_TO_PARTY, false, false);
		
		CharacterShuffler.shuffleCharacters(GameType.FE8, cd, tl, rng, sourceFile, pd, freeSpace, chapterData, classData, shufflingOptions, inventoryOptions, itemData);
	...
	public static void mainFE7(String[] args) throws IOException {
		...
		tl.allowTextChanges = true;
		PortraitDataLoader pd = new PortraitDataLoader(FE7Data.portraitProvider, sourceFile);
		...
		CharacterShufflingOptions shufflingOptions = new CharacterShufflingOptions(CharacterShufflingOptions.ShuffleLevelingMode.AUTOLEVEL, true, 50, Arrays.asList("fe8chars.json", "fe6chars.json"));
		...
		CharacterShuffler.shuffleCharacters(GameType.FE7, cd, tl, rng, sourceFile, pd, freeSpace, chapterData, classData, shufflingOptions, inventoryOptions, itemData);
		DiffCompiler compiler = new DiffCompiler();
		pd.compileDiffs(compiler);
		tl.commitChanges(freeSpace, compiler);
		cd.compileDiffs(compiler);
	...
	public static void mainFE6(String[] args) throws IOException {
		...
		tl.allowTextChanges = true;
		PortraitDataLoader pd = new PortraitDataLoader(FE6Data.portraitProvider, sourceFile);
		FreeSpaceManager freeSpace = new FreeSpaceManager(FEBase.GameType.FE6, FE7Data.InternalFreeRange, sourceFile);
		Random rng = new Random(SeedGenerator.generateSeedValue(SeedGenerator.generateRandomSeed(), 45648997));
		ChapterLoader chapterData = new ChapterLoader(GameType.FE6, sourceFile);
		CharacterShufflingOptions shufflingOptions = new CharacterShufflingOptions(CharacterShufflingOptions.ShuffleLevelingMode.AUTOLEVEL, true, 50, Arrays.asList("fe8chars.json", "fe7chars.json"));
		...
		CharacterShuffler.shuffleCharacters(GameType.FE6, cd, tl, rng, sourceFile, pd, freeSpace, chapterData, classData, shufflingOptions, inventoryOptions, itemData);
		DiffCompiler compiler = new DiffCompiler();
		pd.compileDiffs(compiler);
		tl.commitChanges(freeSpace, compiler);
		cd.compileDiffs(compiler);
	...
	public static void mainGENERATOR(String[] args) throws IOException {
		FileHandler sourceFile = new FileHandler("C:\\Users\\Marvin\\Desktop\\Yune\\JP.gba");
		CharacterDataLoader cd = new CharacterDataLoader(FE6Data.characterProvider, sourceFile);
		PortraitDataLoader pd = new PortraitDataLoader(FE6Data.portraitProvider, sourceFile);
		...
		for (GBAFECharacterData c : cd.canonicalPlayableCharacters(true)) {
			GBACrossGameData chara = new GBACrossGameData(capital(c.displayString()), "portraits/FE6/" + c.displayString().toLowerCase(), "","","", 
					FE6Data.CharacterClass.valueOf(c.getClassID()), c.getLevel(), 
					new GBAFEStatDAO(c.getBaseHP(), c.getBaseSTR(), c.getBaseSKL(), c.getBaseSPD(), c.getBaseDEF(), c.getBaseRES(), c.getBaseLCK()), 
					new GBAFEStatDAO(c.getHPGrowth(), c.getSTRGrowth(), c.getSKLGrowth(), c.getSPDGrowth(), c.getDEFGrowth(), c.getRESGrowth(), c.getLCKGrowth()), 
					new int[] {c.getSwordRank(), c.getLanceRank(), c.getAxeRank(), c.getBowRank(), c.getStaffRank(), c.getAnimaRank(), c.getLightRank(), c.getDarkRank()},
					c.getConstitution(), 
					pd.getPortraitDataByFaceId(c.getFaceID()).getFacialFeatureCoordinates()
					);
...
//		writer.close();
	...
	public static void mainGENERATORFE7(String[] args) throws IOException {
		FileHandler sourceFile = new FileHandler("C:\\Users\\Marvin\\Desktop\\Yune\\fe7.gba");
		CharacterDataLoader cd = new CharacterDataLoader(FE7Data.characterProvider, sourceFile);
		PortraitDataLoader pd = new PortraitDataLoader(FE7Data.portraitProvider, sourceFile);
		...
		for (GBAFECharacterData c : cd.canonicalPlayableCharacters(true)) {
			...
			GBACrossGameData chara = new GBACrossGameData(capital(c.displayString()), "portraits/FE7/" + c.displayString().toLowerCase()+".png", "","",sb.toString().toUpperCase(), 
	...
	public static void mainGENERATORFE8(String[] args) throws IOException {
		FileHandler sourceFile = new FileHandler("C:\\Users\\Marvin\\Desktop\\Yune\\fe8.gba");
		CharacterDataLoader cd = new CharacterDataLoader(FE8Data.characterProvider, sourceFile);
		PortraitDataLoader pd = new PortraitDataLoader(FE8Data.portraitProvider, sourceFile);
		...
		for (GBAFECharacterData c : cd.canonicalPlayableCharacters(true)) {
			...
			GBACrossGameData chara = new GBACrossGameData(capital(c.displayString()), "portraits/FE8/" + c.displayString().toLowerCase()+".png", "","",sb.toString().toUpperCase(), 
